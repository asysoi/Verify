...
public class ReportController {
	...
	private CertService certService;
		...
		public String openConfig(ModelMap model) {
			...
			model.addAttribute("headermap", rmanager.getDownloadconfig().getHeadermap());
		...
		public void XSLFileDownload(HttpSession session,
			...
			try {
				...
				List certs = certService.readCertificates(
						"dataload".equals(rmanager.getOrderby()) ?  "issuedate" : rmanager.getOrderby() , 
						rmanager.getOrder(), builder);
				...
				(new XSLWriter()).makeWorkbook(certs,
						...
						rmanager.getDownloadconfig().getFields(), "Лист Сертификатов").write(
	...
	public String reportcerts(
		...
		filter.setConditionValue("DATEFROM", "DATELOAD", ">=", datefrom==null ? "" : datefrom , FieldType.DATE);
		filter.setConditionValue("DATEFTO", "DATELOAD", "<=", dateto==null ? "" : dateto , FieldType.DATE);
  	    filter.setConditionValue("OTD_NAME", "OTD_NAME", "=", "", FieldType.STRING);
	...
	private ViewManager initViewManager(ModelMap model) {
		...
		rmanager.setHnames(new String[] { "Дата загрузки", "Номер Сертификата", "Отделение",
